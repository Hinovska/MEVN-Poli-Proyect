...
function ModelEngine() {
  ...
  /**Subscribe MQTT */
  self.Subscribe = () => {
    self.mqtt_client.subscribe("Response/LEFT", { qos: 0 }, (error) => {
      if (!error) {
        console.log("Mqtt conectado por ws - Done  Topic: Response/LEFT");
      } else {
        console.log("Mqtt conectado por ws - Fail MAO Topic: Response/LEFT");
      }
    });
    /**CallBack MQTT */
